...
def draw_dfa_starts_with(keyword):
    ...
    for i in range(len(keyword) + 1):
        ...
        f.node(f'q{i}', shape=shape, style='filled', fillcolor='lightgreen' if shape == 'doublecircle' else 'white')
...
def check_and_display():
    ...
    if found:
        ...
        # Run processing tests after DFA
        mp_time, mt_time = run_processing_test()
        messagebox.showinfo("Processing Time Results",
                            f"✅ Multiprocessing time: {mp_time:.3f} sec\n🧵 Multithreading time: {mt_time:.3f} sec")
    else:
...
if __name__ == "__main__":
    multiprocessing.freeze_support()  # fixes multiple window issue on Windows

    root = tk.Tk()
    root.title("Keyword DFA Recognizer + Processing Time Tester")
    ...
    tk.Button(root, text="Check & Generate DFA + Run Processing Test",
